		...
		if ($cat['filter_attr'] > 0)
		{
			...
			foreach ($cat_filter_attr AS $key => $value)
			{
				...
				if($temp_name = $db->getOne($sql))
				{
					...
					//$attr_url_value['filter_attr'] = $temp_arrt_url;
			...
			foreach ($filter_attr AS $fkey=>$fval)
			{
				if ($fval)
				{
					...
					//$attr_url_value['filter_attr'] = $temp_arrt_url_ecshop120;
					$condition[]=array(
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.click_count,g.is_promote, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
	...
	if ($sort=='salenum')
	{
		$sql = 'SELECT SUM(o.goods_number) as salenum, g.goods_id, g.goods_name,g.is_promote, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
				...
				'LEFT JOIN ' . $GLOBALS['ecs']->table('order_info') . ' AS i ' .
				"ON i.order_id = o.order_id  AND i.order_status = 1 AND i.shipping_status = 2 AND i.pay_status = 2 ".				
				"WHERE $where $ext group by g.goods_id ORDER BY $sort $order";
				
	}
	//print_r($sql);//排序问题   未解决  jx
	/* 代码增加_end  By  www.68ecshop.com */
