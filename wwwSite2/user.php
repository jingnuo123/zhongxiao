...
'follow_shop','message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
...
'check_answer', 'check_register', 'back_order', 'back_list', 'back_order_detail', 'back_order_act', 'back_replay' ,
'my_comment', 'my_comment_send', 'shaidan_send', 'shaidan_sale','account_security','act_identity','check_phone',
'update_password','re_binding','update_phone','update_email','act_update_email','re_binding_email','ch_email','ck_email','forget_password',
'back_order_detail', 'del_back_order', 'back_order_detail_edit', 'add_huan_goods',
/*余额额支付密码_更改_START_www.68ecshop.com*/
'act_forget_pass','re_pass','auction_list','forget_surplus_password','act_forget_surplus_password','update_surplus_password',
				'act_update_surplus_password','verify_reset_surplus_email','get_verify_code'); //代码修改   By   www.68ecshop.com
/*余额额支付密码_更改_END_www.68ecshop.com*/
...
elseif($action == 'act_edit_img')
{
	...
	$thumb_path=DATA_DIR. '/headimg/' . date('Ym').'/' ;
	$headimg_thumb = $GLOBALS['image']->make_thumb($headimg_original, '120',  '120', $thumb_path);
	$headimg_thumb = $headimg_thumb ? $headimg_thumb : $headimg_original;
	$sql = 'UPDATE ' . $ecs->table('users') . " SET `headimg`='$headimg_thumb'  WHERE `user_id`='" . $_SESSION['user_id'] . "'";
    $db->query($sql);
...
elseif ($action == 'order_detail')
{
    ...
    foreach ($goods_list AS $key => $value)
    {
		...
		$sql_back = "SELECT bg.*, bo.back_type FROM " . $ecs->table('back_goods') . " AS bg " .
					...
					" AND bg.goods_id = " . $value['goods_id'] .
					" AND bg.product_id = " . $value['product_id'] .
	...
	$order['shipping_status_id'] = $order['shipping_status'];  //代码增加  By  www.68ecshop.com
...
elseif ($action == 'back_order_detail')
{
	...
	$sql = 'SELECT shipping_id, shipping_code, shipping_name ' .
	...
	{ 
		...
		$list_backgoods[$back_type_temp]['goods_list'][] = array( 
			'goods_name'        =>$row_backgoods['goods_name'],
			'goods_attr'        => $row_backgoods['goods_attr'],
			'back_goods_number' => $row_backgoods['back_goods_number'],
			'back_goods_money'  => price_format($row_backgoods['back_goods_number'] * $row_backgoods['back_goods_price'],false),
			'status_back'       => $_LANG['bos'][$row_backgoods['status_back']]. 
									($row_backgoods['status_back']=='3' && 
									$row_backgoods['back_type'] && 
									$row_backgoods['back_type'] != '4' ? 
									' (换回商品已寄出，请注意查收) ' : ''),
			'status_refund'     => $_LANG['bps'][$row_backgoods['status_refund']],
			'back_type'         => $row_backgoods['back_type']);		
	}
...
elseif ($action == 'back_order_detail_edit')
{
	...
	if ($shipping_id)
	{
		$sql = "SELECT shipping_name FROM " . $GLOBALS['ecs']->table('shipping')  .
...
elseif ($action == 'back_order')
{
   ...
   $sql = "select og.goods_id, og.goods_name, og.goods_sn, og.goods_number, og.goods_price, og.product_id, og.goods_attr, o.order_id, o.order_sn, o.user_id, o.shipping_time_end ".
				" from ". $GLOBALS['ecs']->table('order_info') ." AS o left join ". $GLOBALS['ecs']->table('order_goods') ." AS og ".
				" on o.order_id=og.order_id where og.goods_id='$goods_id' and og.order_id='$order_id' and og.product_id='$product_id'";
...
elseif ($action == 'back_order_act')
{
	...
	$sql = "insert into ".$ecs->table('back_order')."(order_sn, order_id, goods_id,  user_id, shipping_fee, consignee, address, ".
				"zipcode, mobile, add_time, postscript , back_reason, goods_name, imgs, back_pay, country, province, city, district, back_type, status_back, supplier_id) ".
			   " values('$order_sn', '$order_id', '$goods_id',  '$user_id', '$order_info[shipping_fee]', '$consignee', '$address', ".
				"'$zipcode', '$mobile', '$add_time', '$postscript', '$back_reason', '$goods_name', '$imgs', '$back_pay', '$country', '$province', '$city', '$district', '$back_type', '5', '$order_info[supplier_id]')";
	$db->query($sql);
	...
	//{		
		if ($back_type == 1)
		{
			...
			$sql = "insert into ".$ecs->table('back_goods').
				"(back_id, goods_id, goods_name, goods_sn, product_id, goods_attr, back_type, ".
				"back_goods_number, back_goods_price, status_back ) ".
				" values('$back_id', '$goods_id', '$goods_name', '$goods_sn', '$_REQUEST[product_id_tui]', '$_REQUEST[goods_attr_tui]', '0', ".
				" '$tui_goods_number', '$_REQUEST[tui_goods_price]', '5') ";
			$db->query($sql);
		}	
		if ($back_type == 4)
		{
			...
			$sql = "insert into ".$ecs->table('back_goods').
				"(back_id, goods_id, goods_name, goods_sn, product_id, goods_attr, back_type, ".
				"back_goods_number, back_goods_price, status_back) ".
				" values('$back_id', '$goods_id', '$goods_name', '$goods_sn', '$_REQUEST[product_id_tui]', '$_REQUEST[goods_attr_tui]', '4', '$tui_goods_number', '$_REQUEST[tui_goods_price]', '5') ";
			$db->query($sql);
		...
		if ($back_type == 2)
		{
			...
			{
				$sql = "insert into ".$ecs->table('back_goods').
					"(back_id, goods_id, goods_name, goods_sn, product_id, goods_attr, back_type, status_refund, back_goods_number, status_back) ".
					" values('$back_id', '$goods_id', '$goods_name', '$goods_sn', '$_REQUEST[product_id_tui]', '$_REQUEST[goods_attr_tui]', '1', '9', '$huan_count', '5') ";
				$db->query($sql);
				$parent_id_huan =  $db->insert_id();
				foreach ($_POST['product_id_huan'] AS $pid_key => $pid_huan)
				{
					$sql = "insert into ". $ecs->table('back_goods') ."(back_id, goods_id, goods_name, goods_sn, product_id, goods_attr,  back_type, parent_id, status_refund, back_goods_number, status_back) ".
						   "values('$back_id', '$goods_id', '$goods_name', '$goods_sn',  '$pid_huan', '". $_POST['goods_attr_huan'][$pid_key] ."', '2', '$parent_id_huan', '9', '1', '5')";
					$db->query($sql);
				}
			}			
		 }
		if ($back_type == 3)
		{
			...
			$sql = "insert into ".$ecs->table('back_goods').
				"(back_id, goods_id, goods_name, goods_sn, product_id, goods_attr, back_type, ".
				"back_goods_number, back_goods_price, status_back) ".
				" values('$back_id', '$goods_id', '$goods_name', '$goods_sn', '$_REQUEST[product_id_tui]', '$_REQUEST[goods_attr_tui]', '3', ".
				" '$tui_goods_number', '$_REQUEST[tui_goods_price]', '5') ";
			$db->query($sql);
		}
	  //}
...
elseif ($action == 'follow_shop')
{
    ...
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('supplier_guanzhu').
...
elseif ($action == 'act_edit_payment')
{
    ...
    if($_POST['pay_code'] == 'alipay_bank')
	{
		...
		$sql = "UPDATE " . $ecs->table('order_info') .
			   " SET pay_id='$pay_id', pay_name='$payment_info[pay_name]', pay_fee='$pay_fee', order_amount='$order_amount', defaultbank='$defaultbank'".
...
elseif ($action == 'tg_login_act')
{
	...
	$sql="select tg.*, tt.type_money, tt.use_start_date, tt.use_end_date from ". $ecs->table('takegoods') ." AS tg ".
...
elseif ($action == 'my_comment_send')
{
	...
	$sql = "INSERT INTO " . $ecs->table('comment') . "(comment_type, id_value, email, user_name, content, comment_rank, add_time, ip_address, user_id, status, rec_id, comment_tag, buy_time, hide_username)" .
		   "VALUES ('$comment_type', '$id_value', '$email', '$user_name', '$content', '$comment_rank', '$add_time', '$ip_address', '$user_id', '$status', '$rec_id', '$comment_tag', '$buy_time', '$hide_username')";
	$db->query($sql);
...
elseif($action == 'get_verify_code')
{
    ...
    else
    {
        ...
        $sql = 'INSERT INTO '.$ecs->table('verifycode').
            '(`mobile`, `getip`, `verifycode`, `dateline`) VALUES (\'' .$phone. '\',\'' .real_ip() .'\',\'' .$verifycode. '\',\'' .gmtime(). '\')';
        $db->query($sql);
...
elseif ($action == 'shaidan_save')
{
	...
	$sql = "INSERT INTO " . $ecs->table('shaidan') . "(rec_id, goods_id, user_id, title, message, add_time, status, hide_username)" .
		   "VALUES ('$rec_id', '$goods_id', '$user_id', '$title', '$message', '$add_time', '$status', '$hide_username')";
	$db->query($sql);
	...
	if (is_array($img_srcs))
	{
		foreach ($img_srcs as $i => $src)
		{
			...
			$sql = "INSERT INTO " . $ecs->table('shaidan_img') . "(shaidan_id, `desc`, image, thumb)" .
				   "VALUES ('$shaidan_id', '".$img_names[$i]."', '$src', '$thumb')";
			$db->query($sql);
		}	
	}
...
function get_takegoods_orders($user_id, $num = 10, $start = 0)
{
    ...
    $sql = "SELECT * ".
           " FROM " .$GLOBALS['ecs']->table('takegoods_order') .
...
function get_user_backorders($user_id, $num = 10, $start = 0)
{
    ...
    $sql = "SELECT bo.*, g.goods_name " .
           " FROM " .$GLOBALS['ecs']->table('back_order') ." AS bo left join ". $GLOBALS['ecs']->table('goods') ." AS g ".
		  " on bo.goods_id=g.goods_id  ".
           " WHERE user_id = '$user_id' ORDER BY add_time DESC";
    $res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
